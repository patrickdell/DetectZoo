...
class FastDetectGPTDetector(BaseTextDetector):
    ...
    def _curvature_score(self, text: str) -> tuple[float, dict]:
        ...
        shift_logits = logits[:, :-1, :].squeeze(0)  # [T-1, V]
        shift_labels = ids[:, 1:].squeeze(0)  # [T-1]

        log_probs = F.log_softmax(shift_logits, dim=-1)
        probs = F.softmax(shift_logits, dim=-1)

        # log p(x_i | x_{<i})
        observed_lp = log_probs.gather(1, shift_labels.unsqueeze(-1)).squeeze(-1)  # [T-1]

        # H(p(· | x_{<i})) — entropy at each position
        entropy = -(probs * log_probs).sum(dim=-1)  # [T-1]

        # Per-position curvature: how much better is the observed token
        # than a random sample from the model's own distribution?
        curvature = observed_lp + entropy  # [T-1]

        mean_c = float(curvature.mean())
        std_c = float(curvature.std()) if curvature.numel() > 1 else 1.0
        score = mean_c / max(std_c, 1e-8)

        return score, {
            "mean_curvature": mean_c,
            "std_curvature": std_c,
            "mean_entropy": float(entropy.mean()),
            "mean_log_prob": float(observed_lp.mean()),
        }
